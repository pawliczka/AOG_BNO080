...
{ 
  ...
  IMUv2.begin();
  IMUv2.endCalibration();
  IMUv2.enableRotationVector(50); //Send data update every 50ms
...
//    XeRoll = G * (rollK - Zp) + Xp;
    ...
    if (IMUv2.dataAvailable()) 
    {
      float yaw_degrees = IMUv2.getYaw() * 180.0 / PI; // conversion to degrees
      if( yaw_degrees < 0 ) yaw_degrees += 360.0;
      yaw_degrees = 360 - yaw_degrees;
      yaw_degrees16 = int(yaw_degrees * 16);
      float roll_degrees = IMUv2.getRoll() * 180.0 / PI;
...
//        Serial.print("0,"); 
      ...
      Serial.print((int)yaw_degrees16);       //heading in degrees * 16 from BNO
      Serial.print(",");
      Serial.print((int)XeRoll);          //roll in degrees * 16
      Serial.print(",");
